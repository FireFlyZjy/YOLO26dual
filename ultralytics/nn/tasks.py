...
from ultralytics.utils.loss import (
    # [YOLO26] 旧的 E2EDetectLoss 导入注释保留
    # E2EDetectLoss,
    E2ELoss,  # [YOLO26] 新增: ProgLoss渐进式损失
...
class BaseModel(torch.nn.Module):
    ...
    def loss(self, batch, preds=None):
        ...
        # [YOLO26] 新增: 每个batch更新ProgLoss渐进权重 (o2m: 0.8 -> 0.1)
...
class DetectionModel(BaseModel):
    ...
    def init_criterion(self):
        ...
        # [YOLO26] 使用 E2ELoss (ProgLoss渐进式损失 + STAL)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    # [YOLO26] 新增: 从YAML顶层读取 end2end 和 reg_max
    nc, act, scales, end2end = (d.get(x) for x in ("nc", "activation", "scales", "end2end"))
    reg_max = d.get("reg_max", 16)  # [YOLO26] DFL bins, 默认16(YOLO11), YOLO26设为1(无DFL)
...
#-------------------------------------------------- YOLOv13 end------------------------------------------------------
            # print(args,c2)
        # [YOLO26] ===== 旧的Detect参数传递注释保留 =====
        # elif m in ((WorldDetect,ImagePoolingAttn) + DETECT_CLASS + V10_DETECT_CLASS + SEGMENT_CLASS + POSE_CLASS + OBB_CLASS):
        #     args.append([ch[x] for x in f])
        # [YOLO26] ===== 注释结束 =====
        # [YOLO26] 新增: 标准Detect类显式传递reg_max和end2end (从YAML顶层读取)
        elif m in ((WorldDetect,ImagePoolingAttn) + DETECT_CLASS + SEGMENT_CLASS + POSE_CLASS + OBB_CLASS):
            args.extend([reg_max, end2end, [ch[x] for x in f]])  # [YOLO26] reg_max和end2end来自YAML顶层
            ...
            if m in (Pose_LSCD,
                     OBB_LSCD, ):
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in DETECT_CLASS + SEGMENT_CLASS + POSE_CLASS + OBB_CLASS:
                m.legacy = legacy
        # [YOLO26] v10Detect单独处理: 只传channels (v10Detect自带end2end=True)
        elif m in V10_DETECT_CLASS:
            args.append([ch[x] for x in f])
