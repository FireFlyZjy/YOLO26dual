...
from ultralytics.utils.tal import TORCH_1_10, dist2bbox, dist2rbox, make_anchors, TaskAlignedAssigner, bbox2dist
...
class Detect(nn.Module):
    ...
    该类实现了YOLO模型中用于预测边界框和类别概率的检测头，
    支持训练和推理两种模式，并具备可选的端到端检测能力。

    属性:
        dynamic (bool): 强制网格重建。
        export (bool): 模型导出模式标志。
        format (str): 导出格式。
        end2end (bool): 端到端检测模式。
        max_det (int): 单张图片最大检测数量。
        shape (tuple): 输入特征图形状。
        anchors (torch.Tensor): 先验锚框。
        strides (torch.Tensor): 特征图步长。
        legacy (bool): 兼容v3/v5/v8/v9/v11模型的旧版模式。
        xyxy (bool): 输出坐标格式，xyxy或xywh。
        nc (int): 目标类别数量。
        nl (int): 检测层数量。
        reg_max (int): DFL（分布焦点损失）通道数。
        no (int): 每个锚框的输出维度。
        stride (torch.Tensor): 构建时计算的步长。
        cv2 (nn.ModuleList): 边界框回归卷积层。
        cv3 (nn.ModuleList): 类别分类卷积层。
        dfl (nn.Module): 分布焦点损失层。
        one2one_cv2 (nn.ModuleList): 一对一检测的边界框回归卷积层。
        one2one_cv3 (nn.ModuleList): 一对一检测的类别分类卷积层。

    方法:
        forward: 前向传播，返回模型预测结果。
        bias_init: 初始化检测头偏置。
        decode_bboxes: 从预测结果中解码边界框。
        postprocess: 对模型预测结果进行后处理。
    ...
    legacy = False  # 兼容v3/v5/v8/v9模型
    xyxy = False  # 输出坐标格式 xyxy 或 xywh

    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        """初始化YOLO检测层，指定目标类别数和输入通道数。

        参数:
            nc (int): 目标类别数量。
            reg_max (int): DFL最大通道数。
            end2end (bool): 是否使用端到端无NMS检测。
        ...
        self.no = nc + self.reg_max * 4  # 每个锚框的总输出维度
        self.stride = torch.zeros(self.nl)  # 构建时计算的步长
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))  # 卷积通道数
        # 边界框回归分支卷积层
    ...
    @property
    def one2many(self):
        """返回一对多检测头组件，兼容v3/v5/v8/v9/v11模型"""
    ...
    def forward_head(
        ...
        """拼接并返回预测的边界框和类别概率"""
        if box_head is None or cls_head is None:  # 融合推理模式
            return dict()
        bs = x[0].shape[0]  # 批次大小
        # 拼接所有检测层的边界框预测
    ...
    def forward(
        ...
        """前向传播，拼接并返回边界框和类别概率预测结果"""
        # 一对多检测头前向传播
        preds = self.forward_head(x, **self.one2many)
        # 👇 端到端无NMS核心代码：添加一对一检测头分支
        if self.end2end:
        ...
        if self.training:
            return preds
        # 👇 推理时用 one2one 输出（无NMS）：解码预测结果
        y = self._inference(preds["one2one"] if self.end2end else preds)
        # 端到端模式：内置后处理（含topk，不再需要外部NMS）
        if self.end2end:
    ...
    def _inference(self, x: dict[str, torch.Tensor]) -> torch.Tensor:
        """
        从多尺度特征图中解码预测的边界框和类别概率
        端到端后处理：
        直接取 topk 最优框，**不需要 NMS**
        ...
        # 拼接边界框和经过sigmoid激活的类别分数
        return torch.cat((dbox, x["scores"].sigmoid()), 1)

    def _get_decode_boxes(self, x: dict[str, torch.Tensor]) -> torch.Tensor:
        """根据锚框和步长获取解码后的边界框"""
        shape = x["feats"][0].shape  # 特征图形状 BCHW
        # 动态更新锚框和步长
    ...
    def postprocess(self, preds: torch.Tensor) -> torch.Tensor:
        ...
        参数:
            preds (torch.Tensor): 原始预测结果，形状为(batch_size, num_anchors, 4 + nc)

        返回:
            (torch.Tensor): 后处理后的预测结果
        """
        # 拆分边界框和分数
    ...
    def get_topk_index(self, scores: torch.Tensor, max_det: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        参数:
            scores (torch.Tensor): 分数张量
            max_det (int): 单张图片最大检测数

        返回:
            (torch.Tensor, torch.Tensor, torch.Tensor): Top分数、类别索引、筛选索引
        """
...
class Detect(nn.Module):
    ...
    # [YOLO26] 旧的__init__签名注释保留:
    # def __init__(self, nc=80, ch=()):
    #     self.reg_max = 16  # hardcoded DFL
    # [YOLO26] 新增: reg_max和end2end从YAML顶层传入，对齐YOLO26
    def __init__(self, nc=80, reg_max=16, end2end=False, ch=()):
        """Initializes the YOLO detection layer with specified number of classes and channels."""
        super().__init__()
        self.nc = nc  # number of classes
        self.nl = len(ch)  # number of detection layers
        self.reg_max = reg_max  # [YOLO26] DFL channels, 从YAML传入 (YOLO26=1, YOLO11=16)
        self.no = nc + self.reg_max * 4  # number of outputs per anchor
        self.stride = torch.zeros(self.nl)  # strides computed during build
        self._end2end = end2end  # [YOLO26] 私有属性，通过property访问
        ...
        if self._end2end:  # [YOLO26] 使用私有属性判断
            self.one2one_cv2 = copy.deepcopy(self.cv2)
            self.one2one_cv3 = copy.deepcopy(self.cv3)

    # [YOLO26] 新增: end2end 属性 getter/setter (对齐YOLO26)
    @property
    def end2end(self):
        """Checks if the model has one2one for end-to-end detection."""
...
#------------------------------------------------------------ From 魔鬼面具 (https://github.com/z1069614715/objectdetection_script) ------------------------------------------------
...
class E2ELoss:
    ...
    def __init__(self, model, loss_fn=v8DetectionLoss):
        """初始化端到端损失函数，使用传入的模型配置一对多和一对一检测损失。"""
        # 一对多检测损失，tal_topk=10
        self.one2many = loss_fn(model, tal_topk=10)
        # 一对一检测损失，tal_topk=7，tal_topk2=1（严格一对一匹配）
        self.one2one = loss_fn(model, tal_topk=7, tal_topk2=1)
        self.updates = 0  # 训练更新步数
        self.total = 1.0  # 损失权重总和
        # 初始损失权重
        self.o2m = 0.8     # 一对多损失初始权重
        self.o2o = self.total - self.o2m  # 一对一损失初始权重
        self.o2m_copy = self.o2m  # 保存初始一对多权重
        # 最终损失权重
        self.final_o2m = 0.1  # 训练结束时一对多损失的最终权重

    def __call__(self, preds: Any, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        """计算边界框、类别、dfl损失的加权和，并乘以批次大小。"""
        # 解析模型输出
    ...
    def update(self) -> None:
        """根据衰减策略更新一对多和一对一损失的权重。"""
        self.updates += 1  # 步数+1
        self.o2m = self.decay(self.updates)  # 更新一对多权重
        self.o2o = max(self.total - self.o2m, 0)  # 更新一对一权重

    def decay(self, x) -> float:
        """根据当前更新步数计算一对多损失的衰减权重。"""
        # 线性衰减公式：从初始权重逐步降低到最终权重
