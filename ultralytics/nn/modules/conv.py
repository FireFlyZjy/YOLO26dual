...
class ChannelToNumber(nn.Module):
    # 通道转批次模块：将4通道输入拆分并重组为(batch_size*2, 3, H, W)
    def __init__(self):
        super(ChannelToNumber, self).__init__()

    def forward(self, x):
        n1 = x[:, :3, :, :]
        n2 = x[:, 3:4, :, :].expand(-1, 3, -1, -1)  # 将第4个通道扩展成3通道
        combined_output = torch.cat((n1, n2), dim=0)  # 将结果拼接成(batch_size*2, 3, H, W)
        return combined_output

class NumberToChannel(nn.Module):
    # 批次转通道模块：将(batch_size*2, C, H, W)重组为(batch_size, C*2, H, W)
    def __init__(self):
        super(NumberToChannel, self).__init__()

    def forward(self, x):

        x1, x2 = torch.chunk(x, 2, dim=0)  # 按照batch size分离成两个张量
        combined_output = torch.cat((x1, x2), dim=1)  # 将两个张量按通道合并   通道数变为原来的2倍, H, W)
        return combined_output
