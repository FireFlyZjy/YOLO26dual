...
from typing import Any  # [YOLO26] E2ELoss 类型注解需要
...
'''
# yolo26.yaml设置 reg_max = 1 控制移除DFL，用L1loss代替
...
class v8DetectionLoss:
    ...
    # [YOLO26] 旧签名: def __init__(self, model, tal_topk=10):
    def __init__(self, model, tal_topk=10, tal_topk2=None):  # [YOLO26] 新增 tal_topk2 (STAL)
        ...
        self.assigner = TaskAlignedAssigner(
            ...
            alpha=0.5,
            beta=6.0,
            stride=self.stride.tolist(),  # [YOLO26] 显式传入stride
            topk2=tal_topk2,  # [YOLO26] STAL: topk2辅助筛选
        )
    ...
    # [YOLO26] 新增: parse_output 方法 (E2ELoss需要)
    def parse_output(
    ...
    # [YOLO26] 新增: loss 方法 (E2ELoss调用此方法而非__call__)
    def loss(self, preds: dict[str, torch.Tensor] | list[torch.Tensor], batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        """Calculate detection loss using assigned targets. Called by E2ELoss.

        Handles the existing raw list format from Detect.forward_end2end.
        """
...
# [YOLO26] ===== 旧的 E2EDetectLoss (YOLOv10风格) 注释保留 =====
# class E2EDetectLoss:
#     """Criterion class for computing training losses."""
#
#     def __init__(self, model):
...
# [YOLO26] ===== 旧的 E2EDetectLoss 注释结束 =====
# [YOLO26] 新增: E2ELoss (ProgLoss渐进式损失 + STAL小目标动态分配)
class E2ELoss:
    """Criterion class for computing training losses for end-to-end detection.

    Uses progressive loss weighting (ProgLoss): one2many weight decays from 0.8 to 0.1
    over training, while one2one weight increases accordingly. This compensates for the
    loss of localization precision when DFL is removed.

    STAL (Small Target Adaptive Labeling): one2one uses topk=7 -> topk2=1 two-stage
    filtering, giving small targets more matching opportunities.
    """
    ...
    def __init__(self, model, loss_fn=v8DetectionLoss):
        ...
        Args:
            model: DetectionModel instance (must be de-paralleled).
            loss_fn: Loss function class for each branch (default: v8DetectionLoss).
        """
        self.one2many = loss_fn(model, tal_topk=10)
        self.one2one = loss_fn(model, tal_topk=7, tal_topk2=1)  # [YOLO26] STAL: topk=7 -> topk2=1
        self.updates = 0
        self.total = 1.0
        # init gain (初期权重: one2many=0.8, one2one=0.2)
        self.o2m = 0.8
...
class E2EDetectLoss(E2ELoss):
    """Backward-compatible alias for E2ELoss, used for loading old YOLOv10 checkpoints."""
    pass
