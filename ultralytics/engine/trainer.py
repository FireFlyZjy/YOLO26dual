...
from ultralytics.utils.torch_utils import (
    ...
    unset_deterministic,
    unwrap_model,  # [YOLO26] MuSGD build_optimizer 需要
)
...
class BaseTrainer:
    ...
    # [YOLO26] ===== 旧的 build_optimizer 注释保留 =====
    ...
    #     return optimizer
    # [YOLO26] ===== 旧的 build_optimizer 注释结束 =====

    # [YOLO26] 新增: MuSGD 优化器支持 (融合 Muon + SGD)
    def build_optimizer(self, model, name="auto", lr=0.001, momentum=0.9, decay=1e-5, iterations=1e5):
        """Construct an optimizer for the given model, with MuSGD support.
        ...
        """
        g = [{}, {}, {}, {}]  # [YOLO26] 4组参数分组: weight_decay, no_decay, bias, muon_params
        ...
        if name == "auto":
            ...
            # [YOLO26] auto模式: iterations>10000 -> MuSGD
            name, lr, momentum = ("MuSGD", 0.01, 0.9) if iterations > 10000 else ("AdamW", lr_fit, 0.9)
            self.args.warmup_bias_lr = 0.0  # no higher than 0.01 for Adam

        use_muon = name == "MuSGD"  # [YOLO26] 是否启用Muon正交化更新
        for module_name, module in unwrap_model(model).named_modules():
            for param_name, param in module.named_parameters(recurse=False):
                fullname = f"{module_name}.{param_name}" if module_name else param_name
                if param.ndim >= 2 and use_muon:
                    g[3][fullname] = param  # [YOLO26] muon params: ndim>=2 的权重用Muon更新
                    ...
                    g[0][fullname] = param
        if not use_muon:
            g = [x.values() for x in g[:3]]  # [YOLO26] 非MuSGD模式: 回退到3组参数
        ...
        muon, sgd = (0.2, 1.0)  # [YOLO26] Muon和SGD的权重配比
        if use_muon:
            num_params[0] = len(g[3])  # update number of params
            g[3] = {"params": g[3], **optim_args, "weight_decay": decay, "use_muon": True, "param_group": "muon"}
            import re

            # [YOLO26] MuSGD微调时某些参数用更高学习率
